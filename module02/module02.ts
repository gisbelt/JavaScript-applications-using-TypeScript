...
// uses (&) to separate each type. 
// An intersection allows you to aggregate existing types together 
// to obtain a single type that has all the characteristics you need.
interface Employee {
    employeeID: number;
    age: number;
}
...
// ******************************** 
// literal types 
// The best way to show the use of literal types is with an example. 
// This type definition creates a literal type named testResult, 
// which can contain one of these three string values:
...
// which act just like the string literals above. For example:
